    ...
    /***/ function(module, exports) {
      ...
      Clubber.prototype.resize = function(bins) {
        ...
        for (var i = 0, inc = this.sampleRate / 2 / this.bufferLength; i < this.bufferLength; i++) {
          ...
          var key = Math.floor(17.3123405046 * Math.log(0.12231220585 * freq));
          this.keys[i] = key;
          this.weights[key]++;
        }
        var holeIndex = 0;
      ...
      Clubber.prototype.band = function(config) {
        ...
        var parseConfig = function(config) {
          ...
          if (config) {
            ...
            if (typeof config.template === 'string') {
              var t = [];
              for (var i = 0; i < config.template.length; i++) t.push(parseInt(config.template[i]));
              config.template = t;
            }
            var rect = {
              from: config.from,
        ...
        return function(output, offset) {
          ...
          function smoothFn(v, o, f, snap) {
            ...
            f = Math.min(f, snap);
            if (f < 0) {
              f = v > o ? Math.abs(snap) : -f;
            }
            return f * v + (1 - f) * o;
          }

          var width = config.to - config.from,
            av = cnt ? vsum / cnt : 0;
          var height = rect.high - rect.low,
            _height = config.high - config.low,
            area = width * height;
          ...
          var ocf = Math.floor(config.from / 12),
            oct = Math.ceil(config.to / 12);
          ...
          for (var t = obj.time, step = 1000 / scope.fps, tmax = scope.time; t < tmax; t += step) {
            config.template.forEach(function(k, i) {
              switch (k) {
                default:
                  data[i] = smoothFn(obj.idx, data[i], s[i], snap);
                  break;
                case 1:
                  data[i] = smoothFn(obj.midx, data[i], s[i], snap);
                  break;
                case 2:
                  data[i] = smoothFn(obj.avg, data[i], s[i], snap);
                  break;
                case 3:
                  data[i] = smoothFn(val, data[i], s[i], snap);
                  break;
                case 4:
                  data[i] = smoothFn(cnt && height ? av / height : 0, data[i], s[i], snap);
                  break;
                case 5:
                  data[i] = smoothFn(vsum ? psum / vsum / width : 0, data[i], s[i], snap);
                  break;
                case 6:
                  data[i] = smoothFn(vsum ? (osum / vsum - ocf) / (oct - ocf) : 0, data[i], s[i], snap);
                  break;
                case 7:
                  data[i] = smoothFn(area ? vsum / area : 0, data[i], s[i], snap);
                  break;
                case 8:
                  data[i] = smoothFn((rect.low - config.low) / _height, data[i], s[i], snap);
                  break;
                case 9:
                  data[i] = smoothFn((rect.high - config.low) / _height, data[i], s[i], snap);
                  break;
              }
            });
